...
void FieldFrame::GetMinMax(PMINMAX pmm)
{
	if(*(UINT*)&minmax == 0)
	{
		...
		for(int i = 0; i < vCtrls.GetCount(); i++)
		{
			SIZE csz;
			...
			minmax.szMax.cy += pmm->szMin.cy;
		}
		minmax.szMin.cx += (cdiv=clen) + cspace + space.left + space.right;
		minmax.szMax.cx += (cdiv=clen) + cspace + space.left + space.right;
		
		minmax.szMin.cy += (minmax.szMin.cy * rowspace_min / (100 * vCtrls.GetCount())) * (vCtrls.GetCount() - 1);
		minmax.szMax.cy += (minmax.szMax.cy * rowspace_max / (100 * vCtrls.GetCount())) * (vCtrls.GetCount() - 1);
		minmax.szMin.cy += space.top + space.bottom;
		minmax.szMax.cy += space.top + space.bottom;
		minmax.szDes = minmax.szMin;

		top = new int[vCtrls.GetCount()];
		height = new int[vCtrls.GetCount()];
	}
...
void FieldFrame::Draw(HDC hdc)
{
	...
	if(Style.pFont)
		hf = ::SelectObject(hdc, *Style.pFont);
